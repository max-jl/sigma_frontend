...
function Get_quote () {
    ...
    const today = new Date(get_date().today);
    // Gets difference in date into integer
    let diff_days = Math.ceil(Math.abs(today - date1) / (1000 * 60 * 60 * 24)) - 1;
    ...
    // Debug
    // today_quote = Return_quote_list(143).quote;
...
function write_quote_author(today_quote) {
    ...
    // Twitter api and clipboard text
    // Twitter uses a link to generate a tweet, so this generates the right link
    ...
    // Gives svf and buttons event listeners and attributes
    svg = container.querySelector('svg');
    svg.ariaLabel = "Quote Author";
    // Alt for disabled users
...
// Initialise the quotes gets the quote
...
// Returns the value of the quote list
